...
<div class="space-y-6">
    ...
    <div class="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-3xl p-6 shadow-xl">
            ...
            <div class="flex items-center space-x-2 flex-wrap">
                ...
                @if($workflow->warehouse)
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 font-bold gap-1">
                        ...
                        <span>Facility: {{ $workflow->warehouse->name }}</span>
                    </span>
                @else
        ...
        <div class="flex items-center space-x-3 flex-wrap gap-y-2">
            ...
            <form action="{{ route('workflows.destroy', $workflow->id) }}" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete workflow \'{{ $workflow->name }}\'?')">
                @csrf
                @method('DELETE')
                <button type="submit" class="p-2 bg-rose-500/10 hover:bg-rose-500/20 text-rose-500 border border-rose-500/20 rounded-xl text-xs transition" title="Delete Workflow">
    ...
    <div class="space-y-4">
        <div class="flex items-center justify-between">
            ...
            <span class="text-[10px] text-indigo-500 bg-indigo-500/10 px-2.5 py-1 rounded-full border border-indigo-500/20 flex items-center gap-1.5 font-medium">
        ...
        <div id="statesContainer" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($workflow->states as $state)
                @php
                    ...
                    $themeConfig = [
                        'emerald' => [
                            'card_class' => 'state-card-emerald',
                            'dot_class' => 'bg-emerald-500',
                            'dark_bg' => 'bg-emerald-950/40 border-emerald-500/40 text-emerald-300',
                        ],
                        'amber' => [
                            'card_class' => 'state-card-amber',
                            'dot_class' => 'bg-amber-500',
                            'dark_bg' => 'bg-amber-950/40 border-amber-500/40 text-amber-300',
                        ],
                        'rose' => [
                            'card_class' => 'state-card-rose',
                            'dot_class' => 'bg-rose-500',
                            'dark_bg' => 'bg-rose-950/40 border-rose-500/40 text-rose-300',
                        ],
                        'indigo' => [
                            'card_class' => 'state-card-indigo',
                            'dot_class' => 'bg-indigo-500',
                            'dark_bg' => 'bg-indigo-950/40 border-indigo-500/40 text-indigo-300',
                        ],
                        'purple' => [
                            'card_class' => 'state-card-purple',
                            'dot_class' => 'bg-purple-500',
                            'dark_bg' => 'bg-purple-950/40 border-purple-500/40 text-purple-300',
                        ],
                        'blue' => [
                            'card_class' => 'state-card-blue',
                            'dot_class' => 'bg-blue-500',
                            'dark_bg' => 'bg-blue-950/40 border-blue-500/40 text-blue-300',
                        ],
                        'slate' => [
                            'card_class' => 'state-card-slate',
                            'dot_class' => 'bg-slate-400',
                            'dark_bg' => 'bg-slate-900 border-slate-700 text-slate-300',
                        ],
                    ][$color] ?? [
                        'card_class' => 'state-card-slate',
                        'dot_class' => 'bg-slate-400',
                ...
                <div data-state-id="{{ $state->id }}" class="workflow-state-card {{ $themeConfig['card_class'] }} border rounded-3xl p-5 shadow-xl relative flex flex-col justify-between space-y-4 transition-all duration-200 {{ $themeConfig['dark_bg'] }}">
                    <!-- State Header -->
                    <div class="space-y-2">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center space-x-2.5">
                                <div class="drag-handle cursor-grab active:cursor-grabbing text-slate-400 hover:text-indigo-600 p-1 rounded hover:bg-slate-100/50 transition" title="Drag to reorder position">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 8h16M4 16h16" />
                                    </svg>
                                </div>
                                <span class="w-2.5 h-2.5 rounded-full {{ $themeConfig['dot_class'] }}"></span>
                                <h4 class="font-bold text-base state-title text-white">{{ $state->name }}</h4>
                            </div>

                            <div class="flex items-center space-x-1">
                                @if($state->is_initial)
                                    <span class="px-2 py-0.5 rounded-full text-[9px] bg-amber-500/20 text-amber-600 font-extrabold uppercase border border-amber-500/30">Initial</span>
                                @endif
                                @if($state->is_final)
                                    <span class="px-2 py-0.5 rounded-full text-[9px] bg-emerald-500/20 text-emerald-600 font-extrabold uppercase border border-emerald-500/30">Terminal</span>
                                @endif

                                <!-- Edit State Button -->
                                <button type="button" onclick='openEditStateModal({{ json_encode($state) }})' class="text-slate-400 hover:text-indigo-600 p-1 transition" title="Edit State Step">
                                ...
                                <form action="{{ route('workflows.states.delete', $state->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete state \'{{ $state->name }}\'?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="text-slate-400 hover:text-rose-600 p-1 transition font-bold" title="Delete State Step">&times;</button>
                                </form>
                            </div>
                        </div>

                        <div class="font-mono text-[10px] text-slate-400 flex items-center space-x-1.5">
                            <span>code:</span>
                    ...
                    <div class="pt-3 border-t border-slate-700/50 light:border-slate-200 space-y-2">
                        <span class="text-[10px] font-bold uppercase tracking-wider text-slate-400 block">Outgoing Action Transitions ({{ $state->outgoingTransitions->count() }})</span>

                        <div class="space-y-2">
                            @forelse($state->outgoingTransitions as $t)
                                <div class="state-transition-card bg-slate-950/70 border border-slate-800 rounded-2xl p-2.5 text-xs text-white space-y-1.5">
                                    <div class="flex items-center justify-between">
                                        <span class="font-bold text-indigo-400 flex items-center space-x-1.5">
                                            <span>{{ $t->action_name }}</span>
                                            <svg class="w-3.5 h-3.5 text-slate-400 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>
                                            <span class="text-emerald-400 font-semibold">{{ $t->toState->name }}</span>
                                        </span>

                                        <form action="{{ route('workflows.transitions.delete', $t->id) }}" method="POST" class="inline" onsubmit="return confirm('Remove transition \'{{ $t->action_name }}\'?')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="text-slate-400 hover:text-rose-500 text-xs px-1 font-bold">&times;</button>
                                        </form>
                                    </div>

                                    <div class="flex items-center justify-between text-[10px] text-slate-400 pt-1.5 border-t border-slate-800/80 light:border-slate-100">
                                        <span>Roles: 
                                            @if(empty($t->allowed_roles))
                                                <strong class="text-slate-300 light:text-slate-600">All Org Roles</strong>
                                            @else
                                                <strong class="text-amber-400 light:text-amber-700 bg-amber-500/10 px-1 py-0.5 rounded">{{ implode(', ', $t->allowed_roles) }}</strong>
                                            @endif
                                        </span>
                                        @if($t->requires_note)
                                            <span class="text-amber-400 font-semibold">• Note Req</span>
                                        @endif
                                    </div>
                                </div>
                            @empty
                                <div class="state-empty-box text-[10px] text-slate-400 italic p-2.5 rounded-xl bg-black/20 text-center">
        ...
        <form action="{{ route('workflows.states.store', $workflow->id) }}" method="POST" class="space-y-4">
            ...
            <div>
                ...
                <select name="color" required class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white">
                    ...
                    <option value="blue">Sky Blue (Review)</option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="warehouse_id" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-xs text-white">
                    ...
                    @foreach($warehouses as $wh)
                        <option value="{{ $wh->id }}" {{ $workflow->warehouse_id == $wh->id ? 'selected' : '' }}>{{ $wh->name }} @if($wh->location) - {{ $wh->location }} @endif</option>
                    @endforeach
                </select>
